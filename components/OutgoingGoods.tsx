...
import { MinusCircle, Save, Clock, Eye, Trash2, X, Calendar, Package, FileText, Search, ChevronDown } from 'lucide-react';
...
const OutgoingGoods: React.FC<OutgoingGoodsProps> = ({ items, transactions, onAddTransaction, onDeleteTransaction }) => {
  const getTodayDate = () => {
    const d = new Date();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedItem && Number(quantity) > selectedItem.stock) {
      setError(`Stok tidak mencukupi. Stok saat ini: ${selectedItem.stock} ${selectedItem.unit}`);
      return;
    }
    ...
    onAddTransaction(selectedItemId, Number(quantity), notes, TransactionType.OUT, date);
  ...
  const outgoingHistory = transactions.filter(t => t.type === TransactionType.OUT);
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 animate-fade-in items-start">
      ...
      <div className="xl:col-span-1 bg-white p-6 rounded-xl shadow-sm border border-slate-200 sticky top-0">
        <div className="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800">Barang Keluar</h2>
            <p className="text-slate-500 text-xs">Catat pengiriman atau penggunaan barang.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="relative">
            <label className="block text-sm font-medium text-slate-700 mb-1">Pilih Barang</label>
             ...
             <div 
              ...
              className={`w-full p-2.5 border rounded-lg bg-white text-sm flex items-center justify-between cursor-pointer transition-all ${
                isDropdownOpen ? 'ring-2 ring-orange-500 border-orange-500' : 'border-slate-300 hover:border-orange-400'
              }`}
            >
                  ...
                  <div className="p-2 border-b border-slate-100 bg-slate-50">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-md text-sm focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500"
                      />
                    </div>
                  </div>
                    ...
                    {filteredItems.length > 0 ? (
                      filteredItems.map(item => (
                        <div
                          ...
                          className={`p-2.5 hover:bg-orange-50 cursor-pointer rounded-lg text-sm transition-colors mb-0.5 ${
                            selectedItemId === item.id ? 'bg-orange-50 text-orange-700 ring-1 ring-orange-200' : 'text-slate-700'
                          }`}
                        >
            ...
            {selectedItem && (
               <p className="text-xs text-slate-500 mt-2 flex items-center gap-1">
                 <Package size={12} />
                 Stok Tersedia: <span className="font-bold text-slate-700">{selectedItem.stock} {selectedItem.unit}</span>
               </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 text-sm ${error && !quantity ? 'border-red-300 bg-red-50' : 'border-slate-300'}`}
                  placeholder="0"
                  required
                />
                {selectedItem && (
            ...
            <div>
               ...
               <input 
                  ...
                  className="w-full p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 text-sm"
                  required
               />
            </div>
          </div>
          {error && <p className="text-xs text-red-600 font-medium bg-red-50 p-2 rounded border border-red-100 flex items-center gap-1"><MinusCircle size={12}/> {error}</p>}

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Catatan / Tujuan</label>
            <textarea
              value={notes}
          ...
          <button
            type="submit"
            className="w-full flex justify-center items-center gap-2 bg-orange-600 text-white py-2.5 px-4 rounded-lg hover:bg-orange-700 transition-colors font-medium shadow-sm hover:shadow-md text-sm"
          >
            <Save size={18} />
            Simpan Transaksi
          </button>
              ...
              {outgoingHistory.length > 0 ? (
                ...
                <tr>
                   <td colSpan={4} className="px-6 py-12 text-center text-slate-400">
                      ...
                      <p>Belum ada riwayat transaksi keluar.</p>
                   </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden animate-fade-in">
            <div className="bg-orange-50 px-6 py-4 border-b border-orange-100 flex justify-between items-center">
              <h3 className="font-bold text-orange-900 flex items-center gap-2">
                <Package size={18} /> Detail Barang Keluar
              </h3>
              <button 
                onClick={() => setViewDetail(null)}
                className="text-orange-700 hover:text-orange-900 hover:bg-orange-100 p-1 rounded"
              >
                <X size={20} />
              </button>
            </div>
            <div className="p-6 space-y-4">
               ...
               <div className="flex items-start gap-3">
                  <div className="mt-1 text-slate-400"><MinusCircle size={18} /></div>
                  <div>
                    <p className="text-xs text-slate-500 uppercase">Jumlah Keluar</p>
                    <p className="font-bold text-orange-600 text-lg">-{viewDetail.quantity}</p>
                  </div>
               </div>
               <div className="flex items-start gap-3">
                  <div className="mt-1 text-slate-400"><FileText size={18} /></div>
                  <div>
                    <p className="text-xs text-slate-500 uppercase">Catatan / Tujuan</p>
